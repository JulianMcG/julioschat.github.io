...
async function requestNotificationPermission() {
    if (!('Notification' in window)) {
        ...
        alert('Notifications are not supported in this context. Please open the chat in a new tab to enable notifications.');
        return false;
    }
    ...
    try {
        ...
        if (window.self !== window.top) {
            alert('To enable notifications, please open the chat in a new tab by clicking the link below.');
            const newTabLink = document.createElement('a');
            newTabLink.href = window.location.href;
            newTabLink.target = '_blank';
            newTabLink.textContent = 'Open Chat in New Tab';
            newTabLink.style.color = '#1F49C7';
            newTabLink.style.textDecoration = 'none';
            newTabLink.style.marginTop = '10px';
            newTabLink.style.display = 'block';
            document.querySelector('.notification-settings').appendChild(newTabLink);
            return false;
        }
...
function showNotification(title, body) {
    ...
    // Check if we're in an iframe
    if (window.self !== window.top) {
        // Create a visible notification in the chat instead
